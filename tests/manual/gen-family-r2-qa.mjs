/**
 * Round 2 QA — family premium + familySajuDeep overlay.
 *
 * Run:
 *   npx tsx tests/manual/gen-family-r2-qa.mjs
 *
 * Env:
 *   F2_RR_ID, F2_VIEWER_ID, F2_TAG, F2_OUT
 *   F2_PARENT_TYPE=mother|father (default mother)
 *   F2_CHILD_IS_VIEWER=0|1 (default 0 — viewer is parent)
 *   RELATIONSHIP_FAMILY_NARRATIVE=1 (default)
 */
...
const RR_ID = process.env.F2_RR_ID || "";
const VIEWER_ID =
  process.env.F2_VIEWER_ID || "51e60cca-8596-4634-87e7-ca3b6468b14c";
const LOCALE = "ko-KR";
const OUT =
  process.env.F2_OUT ||
  path.join(process.env.TEMP || "/tmp", "family-r2-qa.json");
const TAG = process.env.F2_TAG || "family-qa";
const PARENT_TYPE =
  process.env.F2_PARENT_TYPE === "father" ? "father" : "mother";
const CHILD_IS_VIEWER =
  process.env.F2_CHILD_IS_VIEWER === "1" ||
  process.env.F2_CHILD_IS_VIEWER === "true";
...
async function pickRrId(sb) {
  ...
  if (rrs?.length) {
    const prefer =
      rrs.find((r) => r.relationship_kind === "family") || rrs[0];
    return prefer.id;
  }
  throw new Error("No relationship_reports found; set F2_RR_ID");
}
...
async function main() {
  ...
  push(`START family r2 QA tag=${TAG}`);
  ...
  const { runFamilyParentChildDeepAnalysis } = await load(
  ...
  const { resolveFamilyRolesFromViewer } = await load(
    "lib/relationship/familyParent/resolveFamilyRoles.ts",
  );
  const { stripFamilyContextOutputForClient } = await load(
    "lib/relationship/familyParent/stripFamilyContextOutputForClient.ts",
  );
  const { adviceHasLeadingEvidenceBridge } = await load(
    "lib/prompts/relationshipPremium/familySajuDeep/index.ts",
  );
  ...
  const roles = resolveFamilyRolesFromViewer({
    viewerReportId: VIEWER_ID || rr.report_id_a,
    reportIdA: rr.report_id_a,
    reportIdB: rr.report_id_b,
    parentType: PARENT_TYPE,
    childIsViewer: CHILD_IS_VIEWER,
  });
  push(
    `roles roleA=${roles.roleA} roleB=${roles.roleB} parentType=${PARENT_TYPE} childIsViewer=${CHILD_IS_VIEWER}`,
  );

  const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
  const t0 = Date.now();
  const payload = await runFamilyParentChildDeepAnalysis(openai, {
    nicknameA: labelA,
    nicknameB: labelB,
    roles,
    parentType: PARENT_TYPE,
  ...
  const overlay = payload.report?.meta?.family_saju_deep || null;
  ...
  const { error: upErr } = await mergeRelationshipPremiumByKind(
    ...
    "family",
    withLocale,
    { relationshipKind: "family", locale: LOCALE },
  );
  if (upErr) throw new Error("persist failed: " + (upErr.message || upErr));
  push("PERSIST_OK kind=family");

  const clientSafe = stripFamilyContextOutputForClient(withLocale);
  const tips = [
    ...(overlay?.section_5_action?.advice_for_parent || []),
    ...(overlay?.section_5_action?.advice_for_child || []),
  ];
  ...
  const qa = {
    ...
    child_is_viewer: CHILD_IS_VIEWER,
    elapsed_ms: elapsed,
    generated_at: new Date().toISOString(),
    has_overlay: !!overlay,
    projections: {
      parent_nickname: payload.report?.meta?.parent_nickname ?? null,
      child_nickname: payload.report?.meta?.child_nickname ?? null,
