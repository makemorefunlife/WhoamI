/**
 * Round 2 QA — work premium + businessSajuDeep overlay.
 *
 * Run:
 *   npx tsx tests/manual/gen-business-r2-qa.mjs
 *
 * Env:
 *   B2_RR_ID, B2_VIEWER_ID, B2_TAG, B2_OUT
 *   RELATIONSHIP_BUSINESS_NARRATIVE=1 (default)
 */
...
const RR_ID = process.env.B2_RR_ID || "";
const VIEWER_ID =
  process.env.B2_VIEWER_ID || "51e60cca-8596-4634-87e7-ca3b6468b14c";
const LOCALE = "ko-KR";
const OUT =
  process.env.B2_OUT ||
  path.join(process.env.TEMP || "/tmp", "business-r2-qa.json");
const TAG = process.env.B2_TAG || "business-qa";
...
async function pickRrId(sb) {
  ...
  if (rrs?.length) {
    const prefer =
      rrs.find((r) => r.relationship_kind === "work") || rrs[0];
    return prefer.id;
  }
  throw new Error("No relationship_reports found; set B2_RR_ID");
}
...
async function main() {
  ...
  push(`START business r2 QA tag=${TAG}`);
  ...
  const { runWorkColleagueDeepAnalysis } = await load(
    "lib/prompts/relationshipPremium/workColleague/index.ts",
  );
  ...
  const { stripWorkContextOutputForClient } = await load(
    "lib/relationship/workColleague/stripWorkContextOutputForClient.ts",
  );
  const { adviceHasLeadingEvidenceBridge } = await load(
    "lib/prompts/relationshipPremium/businessSajuDeep/index.ts",
  );
  ...
  const t0 = Date.now();
  const payload = await runWorkColleagueDeepAnalysis(openai, {
    nicknameA: labelA,
    nicknameB: labelB,
  ...
  const overlay = payload.report?.meta?.business_saju_deep || null;
  ...
  const { error: upErr } = await mergeRelationshipPremiumByKind(
    ...
    "work",
    withLocale,
    { relationshipKind: "work", locale: LOCALE },
  );
  if (upErr) throw new Error("persist failed: " + (upErr.message || upErr));
  push("PERSIST_OK kind=work");

  const clientSafe = stripWorkContextOutputForClient(withLocale);
  const tips = [
  ...
  const qa = {
    ...
    projections: {
      leadership:
        payload.report?.canonical_projections?.leadership_split ?? null,
