/**
 * Round 2 QA — friendship premium + friendSajuDeep overlay.
 *
 * Run:
 *   npx tsx tests/manual/gen-friend-r2-qa.mjs
 *
 * Env:
 *   FR2_RR_ID, FR2_VIEWER_ID, FR2_TAG, FR2_OUT
 *   RELATIONSHIP_FRIEND_NARRATIVE=1 (default)
 */
...
const RR_ID = process.env.FR2_RR_ID || "";
const VIEWER_ID =
  process.env.FR2_VIEWER_ID || "51e60cca-8596-4634-87e7-ca3b6468b14c";
const LOCALE = "ko-KR";
const OUT =
  process.env.FR2_OUT ||
  path.join(process.env.TEMP || "/tmp", "friend-r2-qa.json");
const TAG = process.env.FR2_TAG || "friend-qa";
...
async function pickRrId(sb) {
  ...
  if (rrs?.length) {
    const prefer =
      rrs.find((r) => r.relationship_kind === "friendship") || rrs[0];
    return prefer.id;
  }
  throw new Error("No relationship_reports found; set FR2_RR_ID");
}
...
async function main() {
  ...
  push(`START friend r2 QA tag=${TAG}`);
  ...
  const { runFriendSocialDeepAnalysis } = await load(
    "lib/prompts/relationshipPremium/friendSocial/index.ts",
  );
  ...
  const { stripFriendContextOutputForClient } = await load(
    "lib/relationship/friend/stripFriendContextOutputForClient.ts",
  );
  const { adviceHasLeadingEvidenceBridge } = await load(
    "lib/prompts/relationshipPremium/friendSajuDeep/index.ts",
  );
  ...
  const t0 = Date.now();
  const payload = await runFriendSocialDeepAnalysis(openai, {
    nicknameA: labelA,
    nicknameB: labelB,
  ...
  const overlay = payload.report?.meta?.friend_saju_deep || null;
  ...
  const { error: upErr } = await mergeRelationshipPremiumByKind(
    ...
    "friendship",
    withLocale,
    { relationshipKind: "friendship", locale: LOCALE },
  );
  if (upErr) throw new Error("persist failed: " + (upErr.message || upErr));
  push("PERSIST_OK kind=friendship");

  const clientSafe = stripFriendContextOutputForClient(withLocale);
  const tips = [
  ...
  const qa = {
    ...
    has_overlay: !!overlay,
    projections: {
      treasurer: payload.report?.canonical_projections?.treasurer ?? null,
      travel_planner:
        payload.report?.canonical_projections?.travel_planner ?? null,
