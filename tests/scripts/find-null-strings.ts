...
function findNullKeys(obj: any, path = "") {
  if (!obj || typeof obj !== "object") return;
  for (const key of Object.keys(obj)) {
    const currentPath = path ? `${path}.${key}` : key;
    if (obj[key] === "null" || obj[key] === null) {
      console.log(`FOUND NULL AT: ${currentPath} -> ${obj[key]}`);
    } else if (typeof obj[key] === "string" && obj[key].includes("null")) {
      console.log(`FOUND NULL STRING AT: ${currentPath} -> ${obj[key]}`);
    } else if (typeof obj[key] === "object") {
      findNullKeys(obj[key], currentPath);
    }
  }
}
