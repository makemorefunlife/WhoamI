...
export const HiddenHeartsSection = ({ section, payload, personA, personB, n, debug }: SectionProps) => {
  ...
  return (
    ...
    >
      {panels.length > 0 && (
        <div className="mb-16">
          <SubHeading title={isEn ? "Misunderstanding Translator" : "오해 번역기"} tone="coral" tag="Translator" />
                ...
                <div className="overflow-hidden rounded-2xl border border-rel-line bg-rel-surface shadow-sm">
                  <div className="flex flex-wrap items-center gap-2 border-b border-rel-line bg-rel-taupe-soft/50 px-5 py-3">
                    ...
                    <span className="font-rel-sans text-[11.5px] tracking-[0.08em] text-rel-ink-mute">
                      {isEn ? "'s behavior in the moment" : "가 마주하는 행동"}
                    </span>
                  </div>
                  <div className="grid gap-0 sm:grid-cols-3">
                    <Panel step={isEn ? "The scene" : "장면"} body={p.what} />
                    <Panel
                      step={isEn ? "How it can look" : "이렇게 보일 수 있어요"}
                      body={p.easyReading}
                      tone="coral"
                      className="border-t border-rel-line sm:border-l sm:border-t-0"
                    />
                    <Panel
                      step={isEn ? "What's really going on" : "사실은"}
                  ...
                  {p.helpfulResponse && (
                    <div className="border-t border-rel-line bg-rel-deep-soft/40 px-5 py-4">
                      <div className="flex items-center gap-2 font-rel-sans text-[11px] font-semibold tracking-wide text-rel-deep">
                        ...
                        <span>{isEn ? "Try this instead" : "이렇게 해보세요"}</span>
                      </div>
      ...
      <SubHeading title={isEn ? "Hidden Hearts" : "숨은 마음"} tag="Hidden hearts" />
                ...
                <div className="min-w-0 space-y-7">
                  ...
                  <div>
                    <div className="font-rel-sans text-[10px] uppercase tracking-[0.16em] text-rel-ink-mute">
                      {isEn ? "So this is what they do" : "그래서 하는 행동"}
                    </div>
                  ...
                  <div className="rounded-2xl bg-rel-deep-soft px-6 py-5">
                    <div className="font-rel-sans text-[10px] uppercase tracking-[0.16em] text-rel-deep">
                      {isEn ? "Worth recognizing" : "알아봐 주면 좋은 것"}
                    </div>
      ...
      {payload.storyPlan?.romanticGapBatch?.whenWeNeedEachOtherMost ? (
        <div className="mt-16 rounded-2xl border border-rel-line bg-rel-surface p-6 shadow-sm space-y-4">
          <SubHeading title={isEn ? "The specific moments you need each other most" : "서로를 가장 절실히 필요로 하는 구체적 순간들"} tag="When Needed Most" tone="deep" />
          <div className="grid gap-6 md:grid-cols-2 text-xs">
            <div className="rounded-xl border border-rel-line bg-rel-taupe-soft p-5 space-y-2">
              <PersonTag name={personA} side="a" />
              <p className="font-semibold text-rel-ink mt-1">💡 {isEn ? `When ${personA} needs ${personB}:` : `${josaIGa(personA)} ${josaEulReul(personB)} 필요로 할 때:`}</p>
            ...
            <div className="rounded-xl border border-rel-line bg-rel-taupe-soft p-5 space-y-2">
              ...
              <p className="font-semibold text-rel-ink mt-1">💡 {isEn ? `When ${personB} needs ${personA}:` : `${josaIGa(personB)} ${josaEulReul(personA)} 필요로 할 때:`}</p>
...
export const RepairSection = ({ section, payload, personA, personB, n, dailyLifeSection, debug }: SectionProps) => {
  ...
  return (
    ...
    >
      ...
      {helps.length > 0 && (
        <div>
          <SubHeading title={isEn ? "What Helps Each of You" : "각자에게 도움이 되는 것"} tag="What helps" />
      ...
      {payload.storyPlan?.romanticGapBatch?.emergencySos ? (
        <div className="mt-14 rounded-2xl border border-rel-line bg-rel-surface p-6 shadow-sm space-y-6">
          <SubHeading title={isEn ? "A 3-Step Emergency SOS Script for Right After a Fight" : "싸웠을 때 즉시 꺼내 쓰는 3단계 SOS 응급 대화 대본"} tag="Emergency SOS" tone="coral" />
          <div className="grid gap-6 md:grid-cols-2">
            <div className="rounded-xl border border-rel-line bg-rel-taupe-soft p-5 space-y-3">
              ...
              <p className="text-xs text-v4-bad font-medium">{isEn ? "Trigger: " : "트리거: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosAtoB.trigger}</p>
              <p className="text-xs text-rel-ink-soft">{isEn ? "Don't: " : "하지 말 것: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosAtoB.doNot}</p>
              <div className="space-y-1.5 pt-2 text-xs border-t border-rel-line">
                <p className="font-semibold text-rel-deep">{isEn ? "Open with: " : "말문 열기: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosAtoB.firstLine}</p>
                <p className="font-semibold text-rel-ink">{isEn ? "Share what's underneath: " : "속마음 전하기: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosAtoB.bridgeLine}</p>
                <p className="font-semibold text-v4-good">{isEn ? "Reconnect: " : "다시 연결하기: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosAtoB.reconnectionLine}</p>
              </div>
            </div>
            <div className="rounded-xl border border-rel-line bg-rel-taupe-soft p-5 space-y-3">
              <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-v4-bad font-medium">{isEn ? "Trigger: " : "트리거: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosBtoA.trigger}</p>
              <p className="text-xs text-rel-ink-soft">{isEn ? "Don't: " : "하지 말 것: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosBtoA.doNot}</p>
              <div className="space-y-1.5 pt-2 text-xs border-t border-rel-line">
                <p className="font-semibold text-rel-deep">{isEn ? "Open with: " : "말문 열기: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosBtoA.firstLine}</p>
                <p className="font-semibold text-rel-ink">{isEn ? "Share what's underneath: " : "속마음 전하기: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosBtoA.bridgeLine}</p>
                <p className="font-semibold text-v4-good">{isEn ? "Reconnect: " : "다시 연결하기: "}{payload.storyPlan.romanticGapBatch.emergencySos.sosBtoA.reconnectionLine}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {dailyCards.length > 0 && (
        <div className="mt-14">
          <SubHeading title={isEn ? "Scenes You Actually Run Into" : "현실에서 마주하는 장면들"} tag="Real life" />
                ...
                <div className="h-full rounded-xl border-l-2 border-l-rel-taupe bg-rel-surface px-5 py-5 shadow-sm">
                  ...
                  {c.difference && (
                    <p className="mt-3 font-rel-sans text-[13px] leading-[1.75] text-rel-ink-soft">
                      <span className="font-semibold text-rel-taupe">{isEn ? "Difference · " : "차이 · "}</span>
                      {c.difference}
                    </p>
                  )}
                  {c.agreement && (
                    <p className="mt-2 font-rel-sans text-[13px] leading-[1.75] text-rel-ink-soft">
                      <span className="font-semibold text-rel-deep">{isEn ? "Agreement · " : "합의 · "}</span>
                      {c.agreement}
                    </p>
                  )}
                  {c.usableLine && (
      ...
      {steps.length > 0 && (
        <div className="mt-14">
          <SubHeading title={isEn ? "Sequence to Recover" : "회복 순서"} tone="coral" tag="Sequence" />
      ...
      <div className="rounded-2xl border border-rel-line bg-rel-surface p-6 shadow-sm space-y-4">
        ...
        <div className={listA.length > 0 && listB.length > 0 ? "grid gap-6 md:grid-cols-2" : "grid gap-6"}>
          {listA.length > 0 ? (
            <div className="rounded-xl border border-rel-line bg-rel-taupe-soft p-5 space-y-2">
              <PersonTag name={personA} side="a" />
              <p className="text-xs font-semibold text-rel-ink mt-1">{isEn ? `Expectations for ${personA} to let go of toward ${personB}:` : `${josaIGa(personA)} ${josaEge(personB)} 내려놓아야 할 기대:`}</p>
          ...
          {listB.length > 0 ? (
            <div className="rounded-xl border border-rel-line bg-rel-taupe-soft p-5 space-y-2">
              <PersonTag name={personB} side="b" />
              <p className="text-xs font-semibold text-rel-ink mt-1">{isEn ? `Expectations for ${personB} to let go of toward ${personA}:` : `${josaIGa(personB)} ${josaEge(personA)} 내려놓아야 할 기대:`}</p>
              <ul className="list-disc pl-4 text-xs text-rel-ink-soft space-y-1">
                {listB.map((item, i) => (
                  <li key={i}><strong>{item.title}</strong>: {item.reason}</li>
                ))}
    ...
    >
      ...
      {payload.storyPlan?.romanticGapBatch?.chapter06 ? (
        <div className="mb-8 rounded-2xl border border-rel-line bg-rel-surface p-6 shadow-sm space-y-4">
          <SubHeading title={isEn ? "What You Learn From Each Other & Synergy" : "서로에게서 배우는 점 및 관계 시너지"} tag="Synergy & Learning" tone="deep" />
          <div className="rounded-xl bg-rel-taupe-soft/50 p-5 border border-rel-line text-xs text-rel-ink-soft space-y-2.5">
            {payload.storyPlan.romanticGapBatch.chapter06.roleMatrix.complement ? (
              <p><span className="font-semibold text-rel-deep">{isEn ? "Synergy · " : "시너지 · "}</span>{payload.storyPlan.romanticGapBatch.chapter06.roleMatrix.complement}</p>
            ) : null}
            {payload.storyPlan.romanticGapBatch.chapter06.growth.aLearnsFromB ? (
              <p><span className="font-semibold text-rel-accent">{isEn ? `What ${personA} learns · ` : `배우는 점 (${personA}) · `}</span>{payload.storyPlan.romanticGapBatch.chapter06.growth.aLearnsFromB}</p>
            ) : null}
            {payload.storyPlan.romanticGapBatch.chapter06.growth.bLearnsFromA ? (
              <p><span className="font-semibold text-rel-accent">{isEn ? `What ${personB} learns · ` : `배우는 점 (${personB}) · `}</span>{payload.storyPlan.romanticGapBatch.chapter06.growth.bLearnsFromA}</p>
            ) : null}
          </div>
        </div>
      ) : null}
              ...
              <article className="h-full rounded-2xl border border-rel-line bg-rel-surface p-6 shadow-sm sm:p-7">
                ...
                {g.shadow && (
                  <p className="mt-4 rounded-xl border border-v4-bad/30 bg-v4-bad-soft px-4 py-3 font-rel-sans text-[13px] leading-[1.7] text-rel-ink">
                    <span className="font-semibold text-v4-bad">{isEn ? "When it goes too far " : "과해질 때 "}</span>
                    {g.shadow}
                  </p>
                )}
              </article>
...
export const FutureTimingSection = ({ section, payload, n, debug }: SectionProps) => {
    ...
    >
      ...
      <Reveal>
        <p className="mt-10 max-w-[62ch] font-rel-sans text-[12.5px] leading-[1.8] text-rel-ink-mute">
          {isEn
            ? "This chapter doesn't claim to know a fixed future. It simply shows the directions the two of you could choose from where things stand now."
  ...
  return (
    ...
    >
      ...
      {remember.length > 0 && (
        <Reveal>
          <div className="mt-14 space-y-4">
            <SubHeading title={isEn ? "Lines to Remember" : "기억할 문장"} tone="coral" tag="Remember" />
            {remember.map((r) => (
              <div
                key={r.person}
            ...
            {improvingSignals.length > 0 && (
              <div className="rounded-xl border border-v4-good/30 bg-v4-good-soft px-4 py-4">
                <div className="font-rel-sans text-[10px] uppercase tracking-[0.16em] text-v4-good">
                  {isEn ? "Signs you're improving" : "나아지고 있다는 신호"}
                </div>
                <ul className="mt-2 space-y-1.5">
            ...
            {cautionSignals.length > 0 && (
              <div className="rounded-xl border border-v4-bad/30 bg-v4-bad-soft px-4 py-4">
                <div className="font-rel-sans text-[10px] uppercase tracking-[0.16em] text-v4-bad">
                  {isEn ? "Signs that need caution" : "주의가 필요한 신호"}
                </div>
                <ul className="mt-2 space-y-1.5">
      ...
      <p className="mt-14 border-t border-rel-line pt-6 font-rel-sans text-[12px] leading-[1.85] text-rel-ink-mute">
        {isEn
          ? "This report is a reference to help you understand each other, not a definitive verdict on your relationship. Please treat these interpretations as guidance, not certainty."
          : "이 리포트는 두 사람을 이해하기 위한 참고 자료이며, 관계에 대한 단정적 결론이 아닙니다. 해석은 참고로만 활용해 주세요."}
      </p>
      {debug && <DebugPanel evidenceIds={section.primaryEvidenceIds} />}
    </ChapterSection>
  );
