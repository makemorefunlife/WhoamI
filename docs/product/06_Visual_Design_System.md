...
> **Does not own:** Product meaning, module content, evidence classification, confidence/severity **assignment**, narrative voice, product-specific axes/labels/thresholds/warnings/actions (those remain in `04_Assessment_Philosophy.md` for constitution; `05_Relationship_Product_Bible.md`, domain `05*` blueprints, `05A`, and `02` for product/UX application). Does not own current-implementation status (→ `08`) or design-exploration rationale/product-owner decisions (→ `09`).
> **Inherits:** Assessment constitution from `04_Assessment_Philosophy.md`; product metaphor intent and §13D visualization/evidence contract from the Product Bible; Romantic module architecture and anti-dashboard laws from `05A`.
...
4. Absence beats filler. Insufficient evidence → render nothing, never a thin fake module. (Constitutional: `04_Assessment_Philosophy.md` Laws 4 and 8.)
5. No grade, rank, fate, dashboard-first entry, red/green moral coding, or decorative mysticism. (Constitutional: `04_Assessment_Philosophy.md` Laws 2 and 6.)
