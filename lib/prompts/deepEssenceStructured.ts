...
function buildPartASchema(part01Evidence: Part01EvidenceForPartAPrompt | null | undefined): string {
  ...
  const withSummaryFields = DEEP_ESSENCE_PART_A_SCHEMA.replace(
    ...
    .replace(
      `{ "title": "strength/adaptation overuse pattern (concrete behavior phrase, e.g. 중요한 결정에서 다른 사람의 확신을 먼저 확인하는 방식 — NEVER single-word defect/diagnostic labels like 의존성/완벽주의/불안정함)", "body": "4-6 sentences: strength/adapted capability -> situation where overused -> resulting energy cost / fatigue. NEVER include self-care or wellness advice." }`,
      `{ "title": "strength/adaptation overuse pattern (concrete behavior phrase, e.g. 중요한 결정에서 다른 사람의 확신을 먼저 확인하는 방식 — NEVER single-word defect/diagnostic labels like 의존성/완벽주의/불안정함)", "body": "4-6 sentences: strength/adapted capability -> situation where overused -> resulting energy cost / fatigue. NEVER include self-care or wellness advice."${STRENGTHS_EVIDENCE_FIELD} }`,
    )
    .replace(
      `"optimal": ["fitting environment 1", "fitting environment 2"]`,
...
- layered_identity TITLES (every layer): a title names an OBSERVABLE PATTERN at that specific relational distance, never a personality-type label for the whole person. FORBIDDEN as a title, in any layer, in either language: personality-noun/virtue labels (자유로운 영혼, 배려 깊은 친구, 좋은 사람, 따뜻한 사람, 리더, 공감형, 독립적인 사람, free spirit, caring friend, good person, leader, empath, or any noun-phrase that could be copy-pasted unchanged onto a completely different person's completely different evidence) and the "true self" PHRASE FAMILY — any combination of 진짜/진정한/찐 with 나/모습/자아, or true/real/authentic with self, in ANY title or ANY narrative in this section, including synthesis.narrative (e.g. 진정한 나, 진짜 나, 진짜 모습, 진정한 모습, 본모습, true self, real self, authentic self, who you really are, and any close variant not spelled out here that still implies one layer is more genuine than the others) — the whole point of layered_identity is that no single layer is more "real" than another, so nothing in this section may claim one is. Test before writing: if this exact title could describe a stranger's report unchanged, it has failed — rewrite it as what specifically shows up at this distance for THIS evidence.
- layered_identity.synthesis is REQUIRED whenever at least two of the four layers above are populated — do not treat it as optional or skip it by default; two or more layers is the normal case, and skipping synthesis in that case is a miss, not a safe default. Only omit the field when zero or exactly one layer above is populated (two data points are the minimum for a "change," one is not). It answers a DIFFERENT question than the four layers themselves: not "what is each layer" (already answered above) but "what changes as relational distance changes." Do not restate each layer in turn (that is the exact failure mode to avoid — a synthesis that just lists "at first X, then Y, then Z" without naming the shift between them has failed). Instead name: what shifts between first_impression and known_self/close_private_self; what only becomes visible once someone is close; and, only when natural_self_and_deep_needs is among the populated layers and the evidence genuinely supports it, the contrast between how this person outwardly presents and what they actually need underneath — moving from outward presentation toward the less-visible need, not treating it as a fourth equal impression. Ground every claim ONLY in the layer narratives you already wrote in this same response and their own evidence — never infer a new saju fact, and never invent biography, childhood, family, or work history to explain why the contrast exists. Use conditional language ("may", "can look like") when the contrast between layers is real but not strongly convergent. Keep it short — 3-6 sentences in Korean, or 1-2 short paragraphs in English — this is Part 02's closing insight, not a new chapter (a longer, fuller adaptation narrative belongs to a later, separate part of the report, not here). evidence_refs may reference exact keys from any of the four layers' evidence lists above; never invented, never from another section's evidence.
  FAILED synthesis example (observed live, do not reproduce even in paraphrase — this is a re-list, not a synthesis): "처음에는 차분하고, 친해지면 다정하고, 가까우면 자유로워요." WORKING synthesis example (names a genuine shift, grounded, present-tense, no invented biography): "처음에는 감정을 크게 드러내지 않아 안정적인 사람으로 읽힐 수 있지만, 신뢰가 쌓이면 관계를 세심하게 읽고 반응하는 면이 더 분명해져요. 아주 가까운 관계에서 더 자유롭게 보이는 것은 처음의 모습이 가짜여서가 아니라, 안전하다고 느낄수록 스스로를 조절할 필요가 줄어들기 때문일 수 있습니다."
...
- STRENGTHS & WATCHOUTS (Pattern, Not People & Strength Overuse):
  ...
  - WATCHOUT DIVERSITY: do not return three watchouts that all reduce to the same underlying causal mechanism dressed in different words (e.g. three variations of "checks with others before deciding," even if one is framed as a decision, one as an emotional reaction, and one as a relationship pattern — if the ROOT COST is the same, that's one watchout, not three). Before finalizing, ask what domain each watchout's cost actually lives in — candidate domains include decision load, emotional labor, adaptation load, boundary maintenance, uncertainty handling, relationship calibration, self-suppression — and prefer three watchouts from meaningfully different domains when the evidence supports it. If two candidates genuinely describe the same mechanism, keep the one with stronger evidence and look for a different, still-evidence-backed pattern for the third — never invent a domain the evidence doesn't support just to force variety.
    FAILED example (observed live, do not reproduce even in paraphrase — two of these three are the SAME mechanism restated): "중요한 결정에서 다른 사람의 확신을 먼저 확인하는 방식" + "결정 장애로 인한 우유부단함" — both are "waits for outside confirmation before deciding," just named from two angles (the behavior itself, and its resulting label). A THIRD watchout on top of one of these must come from a genuinely different domain, e.g. an emotional-labor or boundary-maintenance cost — not a third phrasing of the same decision-delay pattern.
...
- FUTURE & DECISION COMPASS (Going Forward with Agency):
  ...
  - checklist (지금 당신에게 가장 중요한 한 가지 / One Next Move):
    ...
    - MUST be explicitly connected to ONE of these already-established anchors from this response — pick whichever is most central to this person's report, and the action must clearly trace back to it, not float free of everything else written above: the growth_edge (summary.growth_edge / growth_edge_real_life_pattern), the primary/widest axis gap (gap_deep_dive), the adaptation tension named in adaptation_story, a recurring decision pattern (playbook.rule or a rows[] entry), or the Recover signal (future.remember[2]). A reader who has just read the rest of the report should be able to see why THIS action, for THIS person, follows from what was already said — if it could be handed to any reader of any report unchanged, it has failed.
  ...
  - closing (조용한 매듭 / Recognition Statement):
    ...
    - NO evaluative praise of the insight itself — closing states the recognition as a plain fact, never judges how meaningful/important/great it is (STRICTLY FORBIDDEN: "참 의미 있어요", "의미 있는 변화예요", "잘 알게 되었어요", "멋진 점이에요", "소중한 발견이에요", "that's truly meaningful", "what a valuable insight", "it's great that you've realized this"). FAILED example (evaluates the insight, do not reproduce even in paraphrase): "이 점을 알게 되었다는 것이 참 의미 있어요." WORKING example (states the same recognition as plain present-tense fact, no judgment of its worth): "지금의 방식과 본래 편한 방식 중 어느 하나만이 진짜인 것은 아니에요. 이제 중요한 선택 앞에서 어느 쪽을 더 사용하고 있는지 알아차릴 수 있습니다."
