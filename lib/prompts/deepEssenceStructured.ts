...
- For every gap_deep_dive entry, follow this exact 4-part semantic contract order and role for each field, and respect the axis's own "Direction fact" line exactly (never contradict it — if it says Current is higher, natural_tendency must describe something genuinely different from current_pattern, not a restatement of the same trait; likewise if Innate is higher):
  1. natural_tendency ("본래 더 편한 방식") — "원래 나는 어떤 방식이 자연스러운가?" (grounded ONLY in [Innate Self evidence]).
  2. current_pattern ("현실에서 익숙해진 방식") — "실제 삶에서는 어떤 방식을 더 많이 사용하게 되었는가?" (grounded ONLY in Current Self evidence and Current x Innate difference).
  3. gives_you ("그 과정에서 얻은 힘") — "그렇게 살아오면서 나는 무엇을 더 잘하게 되었는가?" ACQUIRED/DEVELOPED CAPABILITY ONLY (rendered UI label: "살아오며 생긴 힘"): a concrete, behaviorally specific capability earned while adapting to real-life demands (e.g. "불확실한 상황에서도 직접 기준과 순서를 만들고, 복잡한 일을 체계적으로 정리하는 능력이 발달했어요"). FORBIDDEN in gives_you: describing environment benefits (e.g. FORBIDDEN: "구조적인 환경은 편안함을 줍니다"), repeating innate preference, repeating current pattern without capability framing, or generic praise.
  4. may_cost ("대신 더 많이 쓰게 된 에너지") — "이렇게 적응한 방식을 계속 사용할 때, 나는 무엇 때문에 지치는가?" SUSTAINED ADAPTATION COST / EFFORT / COGNITIVE LOAD ONLY (rendered UI label: "그만큼 드는 에너지"): explain the specific cost of maintaining this adapted way of living (e.g. "모든 것이 충분히 정리되어야 움직일 수 있다고 느끼는 순간에는, 계획을 세우고 확인하는 데 생각보다 많은 에너지를 쓰게 돼요"). MUST be derived strictly from the causal sequence: innate tendency -> current adaptation -> resulting cost. FORBIDDEN in may_cost: generic platitudes (e.g. FORBIDDEN: "규칙이 자주 바뀌면 에너지가 소모될 수 있어요", "변화가 많으면 스트레스를 받습니다") or blaming a bad environment instead of explaining the sustained adaptation cost.
  EVERY gap_deep_dive entry MUST tell this unbroken 4-step causal story: "나는 원래 A가 자연스럽다 -> 하지만 현실에서는 B를 더 많이 사용하게 되었다 -> 그래서 C라는 능력이 발달했다 -> 다만 그 방식을 계속 유지할 때 D라는 에너지가 든다."
  ANTI-GENERIC MANDATE: BANNED cliché filler statements ("규칙이 자주 바뀌면 에너지가 소모될 수 있어요", "변화가 많으면 스트레스를 받을 수 있어요", "안정적인 환경에서는 집중할 수 있어요", "자신의 의견을 표현하면 도움이 돼요"). Connect every claim strictly to THIS person's Current x Innate difference.
