...
function micromanagingSummary(
  ...
  if (notes.includes("a_food_high_vs_b_self_low")) {
    return `${nicknameA}의 추진력(식상)이 ${nicknameB}의 자기 고집(비겁) 영역과 겹쳐, ${nicknameA}가 일방적으로 '이렇게 해'라고 밀어붙이면 ${nicknameB}는 수동적이거나 반발하기 쉽습니다. 마이크로 매니징 피독점 지수 ${pair.micromanaging_poison_index}(${pair.micromanaging_band}).`;
  }
  if (notes.includes("b_food_high_vs_a_self_low")) {
    return `${nicknameB}의 추진력이 ${nicknameA}의 고집 영역과 겹쳐, ${nicknameB}가 세부 지시를 많이 할수록 ${nicknameA}는 에너지가 빠지거나 뒤끝이 남기 쉽습니다. 마이크로 매니징 피독점 지수 ${pair.micromanaging_poison_index}(${pair.micromanaging_band}).`;
  }
  if (notes.includes("both_managerish")) {
    return `양쪽 모두 '내 방식이 맞다'는 추진·관리 성향이 강해, 회의·채팅에서 세부 지시가 겹치면 피로가 빠르게 누적됩니다. 마이크로 매니징 피독점 지수 ${pair.micromanaging_poison_index}(${pair.micromanaging_band}).`;
  }
  return `식상(추진)과 비겁(고집) 밴드 격차로 업무 중 '잔소리·재지시' 누적 위험이 있습니다. 마이크로 매니징 피독점 지수 ${pair.micromanaging_poison_index}(${pair.micromanaging_band}).`;
}
...
function buildMicromanagingPrescription(
  ...
  return {
    ...
    headline: "잔소리 대신 역할 분리 — 마이크로 매니징 조율",
    evidence: {
      ...
      summary: micromanagingSummary(pair, nicknameA, nicknameB),
      snapshot: {
...
function buildLeadershipPrescription(
  ...
  return {
    ...
    headline: "둘 다 리더면 규칙이 리더 — 오피스 주도권 갈등 해소",
    evidence: {
      source: "pair_work_signals",
      signal_paths: [
...
function buildOfficeBaseline(
  ...
  return {
    ...
    headline: "매주 10분 — 협업 운영 점검 (공통)",
    evidence: {
      source: "pair_work_signals",
      signal_paths: ["work_prescription.baseline"],
      summary:
        "pair 교차 신호가 특정 주제에서 강하게 치솟지 않아도, 동료 관계는 작은 오해의 복리가 큽니다. 아래는 예방용 기본 처방입니다.",
      snapshot: {},
    },
    do_list: [
      `금요일 10분 '이번 주 고마웠던 점 1 · 아쉬웠던 점 1' — ${nicknameA}·${nicknameB} 번갈아.`,
      "업무 요청은 채널·기한·완료 기준 3가지를 항상 명시.",
      "갈등 직후 당일 해결 못 하면 '월요일 첫 미팅 안건'으로만 넘기기.",
    ],
    dont_list: [
      "감정 섞인 장문 카톡 연타 — 짧게 쓰고, 복잡하면 5분 통화.",
      "상대 방식을 동료 앞에서 비꼬기.",
      "마감 전날 밤 갑작스런 범위 변경 통보.",
    ],
  };
}

function resolveIntroLine(items: WorkPrescriptionItem[]): string {
  const topics = new Set(items.map((i) => i.topic));
  if (topics.has("micromanaging_coordination") && topics.has("leadership_conflict")) {
    return "추진력과 고집이 동시에 강한 오피스 조합입니다. 아래 Do/Don't는 pair.work 교차 신호에서 뽑았고, 기존 오피스 리포트 서사와 별도로 실천용입니다.";
  }
  if (topics.has("micromanaging_coordination")) {
    return "업무 중 세부 지시·재지시 누적 위험이 감지됐습니다. 역할 분리와 지시 규칙으로 피독점을 낮추세요.";
  }
  if (topics.has("leadership_conflict")) {
    return "주도권 충돌 신호가 있습니다. '누가 맞는지'보다 '누가 이번 안건 결정자인지'를 먼저 정하세요.";
  }
  return "동료 pair 교차 신호 기반 실행 처방입니다. 분석 서사는 기존 섹션을 그대로 보시고, '그래서 뭐 하라고?'는 여기서 확인하세요.";
}
...
export function buildWorkPrescriptions(params: {
  ...
  const items =
    candidates.length > 0
      ? candidates
      : [buildOfficeBaseline(params.nicknameA, params.nicknameB)];

  if (candidates.length > 0 && candidates.length < 2) {
    items.push(buildOfficeBaseline(params.nicknameA, params.nicknameB));
  }
