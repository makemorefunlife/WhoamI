...
function buildSinglePersonWorkProfile(params: {
  ...
  const { name, partnerName, isPersonA, psych, chart, dnaNode, roleNode, locale = "ko-KR" } = params;
  ...
  // -------------------------------------------------------------------
  let identityLabel = dnaNode?.character_title || (isPersonA ? "빠르게 판단하고 직접 움직이는 타입" : "문제를 정리하고 완성도를 높이는 타입");
  let keyTraits: string[] = isPersonA
    ? ["빠른 실행", "현장 대응", "결단", "책임"]
    : ["문제 해결", "구조화", "품질 검토", "체계성"];

  if (decisionScore < 45 || stimulationScore >= 65) {
    identityLabel = dnaNode?.character_title || "빠르게 판단하고 직접 움직이는 타입";
    keyTraits = ["빠른 실행", "현장 대응", "결단", "책임"];
  } else if (structureScore >= 65 || analyticalScore >= 65) {
    identityLabel = dnaNode?.character_title || "문제를 정리하고 완성도를 높이는 타입";
    keyTraits = ["문제 해결", "구조화", "품질 검토", "체계성"];
  } else if (empathyScore >= 65) {
    identityLabel = dnaNode?.character_title || "맥락을 읽고 부드럽게 조율하는 타입";
    keyTraits = ["관계 조율", "맥락 파악", "협력 촉진", "안정 전달"];
  }
  ...
  const workStyleBehaviors: IndividualWorkStyleItem[] = isPersonA
    ? [
        {
          situationLabel: "일을 시작할 때",
          behaviorSummary: "먼저 움직이며 답을 좁혀요",
          microcopy: "핵심 대안을 빠르게 찾고 실행하면서 조정하는 편입니다.",
        },
        {
          situationLabel: "결정할 때",
          behaviorSummary: "판단이 서면 지체 없이 결단해요",
          microcopy: "오래 끌기보다 현장 반응을 보며 빠르게 의사결정을 내립니다.",
        },
        {
          situationLabel: "문제를 풀 때",
          behaviorSummary: "가장 현실적인 대안부터 시도해요",
          microcopy: "이론적인 논의보다 지금 당장 돌릴 수 있는 해법을 찾습니다.",
        },
        {
          situationLabel: "책임을 맡았을 때",
          behaviorSummary: "끝까지 결과를 만들어내요",
          microcopy: "중간에 멈추지 않고 목표한 마감까지 밀고 나가는 힘이 강합니다.",
        },
      ]
    : [
        {
          situationLabel: "일을 시작할 때",
          behaviorSummary: "조건과 기준부터 정리해요",
          microcopy: "범위와 마감 기준을 먼저 잡고 단계적으로 착수하는 편입니다.",
        },
        {
          situationLabel: "결정할 때",
          behaviorSummary: "리스크를 확인하고 신중히 정해요",
          microcopy: "빠진 조항이나 허점이 없는지 검토한 후 결론을 확정합니다.",
        },
        {
          situationLabel: "문제를 풀 때",
          behaviorSummary: "근본 원인을 다각도로 분석해요",
          microcopy: "임시방편보다 구조적인 문제의 본질을 먼저 살핍니다.",
        },
        {
          situationLabel: "책임을 맡았을 때",
          behaviorSummary: "허점 없이 정밀하게 마감해요",
          microcopy: "품질 기준을 엄격하게 수호하며 완성도 높은 결과를 냅니다.",
        },
      ];
  ...
  if (topContributions.length < 2 && roleNode?.weapons?.length) {
    roleNode.weapons.slice(0, 3).forEach((w) => {
      topContributions.push({
        title: w,
        microcopy: "팀 안에서 확실하게 발휘하는 핵심 업무 무기",
      });
    });
  }
  if (topContributions.length < 3) {
    topContributions.push({
      title: "품질 검토",
      microcopy: "결과물의 빠진 조건을 확인하여 리스크 사전에 방지",
    });
  }
  ...
  const suitableWorkTypes = isPersonA
    ? ["방향 설정", "최종 결정", "프로젝트 추진", "대외 협력"]
    : ["기획 구체화", "품질 검토", "문제 해결", "분석 및 체계 설계"];

  // -------------------------------------------------------------------
  // 04. 잘 맞는 역할 · 직무 · 기능 (Real-world Role Families)
  // -------------------------------------------------------------------
  const suitableRoles = isPersonA
    ? ["PM / 프로젝트 리드", "사업 실행 리드", "대외협력 리드"]
    : ["전략기획 역할", "QA/QC 및 프로세스 설계", "분석 및 기획 리드"];
  ...
  // -------------------------------------------------------------------
  const valueKeywords = isPersonA ? ["책임", "완결", "실행 가능성"] : ["원칙", "완성도", "체계성"];

  const internalStandardSentence = dnaNode?.inner_standard || (isPersonA
  ...
  const delegationItems: ConcreteDelegationItem[] = isPersonA
    ? [
        {
          workTitle: "예산·수익 및 디테일 검토",
          partnerName,
          reason: "숫자와 세부 조건까지 정밀하게 확인하는 강점이 상대에게 더 강해요.",
        },
        {
          workTitle: "기획안 세부 체계화",
          partnerName,
          reason: "아이디어를 구조적인 문서와 단계별 프로세스로 정리하는 데 적합해요.",
        },
      ]
    : [
        {
          workTitle: "빠른 현장 조율 및 결단",
          partnerName,
          reason: "즉시 판단하고 다음 행동으로 추진하는 기동력이 상대에게 더 강해요.",
        },
        {
          workTitle: "대외 협상 및 이슈 추진",
          partnerName,
          reason: "외부와 빠르게 미팅을 갖고 판을 끌고 가는 주도권이 상대에게 자연스러워요.",
        },
      ];
  ...
  if (roleNode?.handoff_tasks?.length) {
    roleNode.handoff_tasks.forEach((t: any) => {
      ...
      const title = (!rawTitle || rawTitle === "세부 업무") ? (isPersonA ? "기획·아이디어 확장" : "프로젝트 추진 관리") : rawTitle;
      const reason = typeof t === "object" && t?.reason ? t.reason : `${partnerName}님에게 맡길 때 전체 생산성이 높아지는 영역`;
  ...
  let innateTraits = isPersonA ? ["빠른 판단", "현장 실행"] : ["독립 집중", "원칙 준수"];
  let currentTraits = isPersonA ? ["목표 집중", "체계적 관리"] : ["정밀 계획", "품질 검토"];
  let synthesisSentence = isPersonA
  ...
  if (isSpontaneousInnate && isHighStructuredCurrent) {
    status = "adapted";
    innateTraits = ["유연한 문제 해결", "자유로운 대안"];
    currentTraits = ["높은 계획 구조화", "마감 밀착 관리"];
    synthesisSentence = "본래는 상황에 맞춰 유연하게 움직이는 힘이 강하지만, 지금은 일정과 계획을 꼼꼼하게 관리하는 방식도 많이 익혀 사용하고 있어요.";
  }
...
export function buildIndividualWorkChapterBundle(
  ...
  const { nameA, nameB, psychA, psychB, sajuChartA, sajuChartB, officeReport, locale = "ko-KR" } = params;
  ...
  const mostDifferentInsight = `[${personA.name}]님은 ${personA.suitableWorkTypes[0]}에 강점이 있고, [${personB.name}]님은 ${personB.suitableWorkTypes[0]}에 강점이 있어 서로의 무기가 다릅니다.`;
