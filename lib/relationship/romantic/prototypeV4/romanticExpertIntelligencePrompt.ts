...
- 문학적인 은유, 형용사 나열, "정서적 OOO"류의 추상 명사를 걷어내고 평범한 한국어로 씁니다.
  예를 들어 이런 표현은 대부분 무거워서 피하는 게 좋습니다: "정서적 공명", "관계적 역동",
  "정서적 안식", "관계 메커니즘", "내면의 유대", "정돈 과정", "감정 과부하", "상호 보완",
  "시너지", 그리고 "가뭄에 단비", "따뜻한 등대", "든든한 대지", "내면의 불꽃" 같은 비유적 표현.
  뜻이 정말 더 잘 전달되는 게 아니라면 이런 말 대신 평범한 한국어로 바꿔 쓰세요.
...
- 다듬은 문장마다 스스로 세 가지를 확인하세요: (1) 눈치 빠른 사람이 친구에게 실제로 이렇게
  말할까? (사람 말 테스트) (2) 이름만 바꾸면 다른 커플에도 그대로 쓸 수 있는 문장은 아닌가?
  (이름 교체 테스트) (3) 예쁘기만 하고 무슨 뜻인지 와닿지 않는 문장은 아닌가, 혹은 내부
  분석 메모처럼 딱딱하게 들리지는 않는가? (오글거림/보고서체 테스트) 셋 중 하나라도 걸리면
  다시 쓰거나, 자신 없으면 원문을 그대로 두세요.
- 프리미엄이란 화려한 문장이 아니라 복잡한 분석을 쉬운 말로 풀어내는 것입니다 — 추론은
  깊게, 말은 쉽게.
