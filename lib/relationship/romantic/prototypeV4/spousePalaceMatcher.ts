...
export function resolveSpousePalaceProfile(
  ...
  const profile = getTenGodRomanticProfile(rawTenGodCode);
...
export function resolveBilateralPartnerPreferenceMatchFromCe(params: {
  ...
  const seekerSpousePalace: SpousePalaceProfile =
    seekerCe.spousePalaceProfile ?? {
      ...
      profile: getTenGodRomanticProfile("bigyeon"),
      ...
      distortionWhenStressed: "자존심을 굽히지 않고 팽팽하게 맞서며 작은 타협조차 양보나 굴복으로 여길 수 있음",
      provenance: {
  ...
  switch (seekerTg) {
    case "bigyeon": {
      recognition = `${topicParticle(seekerName)} ${partnerName}의 흔들림 없는 중심과 담백한 태도에 자연스럽게 마음이 움직입니다.`;
      emotionalMeaning = `일방적으로 의존하거나 상대에게 휘둘리지 않고, 서로의 고유한 영역을 온전히 존중하며 대등하게 걸어갈 수 있는 동반자를 바라기 때문입니다.`;
      scene = `서로 각자의 일과 목표에 몰입하다가도, 하루 끝에 마주 앉아 대등한 눈높이에서 서로의 선택을 담담하게 응원하는 순간`;
      pairSpecificEffect = `서로에게 짐이 되지 않는 건강한 거리감과 자립적인 파트너십을 형성합니다.`;
      tensionBridge = `두 사람 모두 자기만의 기준이 확고할 때, 사소한 의견 차이에서도 양보를 굴복으로 여겨 팽팽한 대치가 길어질 수 있습니다.`;
      ...
      if (partnerStemEl === "earth" || partnerStemEl === "metal" || partnerDominantElement === "earth" || partnerDominantElement === "metal") {
        ...
        supportingReasons.push({
          text: `${partnerName}의 흔들림 없는 묵묵함과 자립적인 태도가 ${subjectParticle(seekerName)} 바라는 대등하고 건강한 동반자 관계를 충족시킵니다.`,
        ...
        supportingReasons.push({
          text: `${partnerName}의 배려 깊은 다정함이 ${seekerName}의 독립적인 영역을 침범하지 않고 편안한 거리를 지켜줍니다.`,
    ...
    case "geopjae": {
      recognition = `${topicParticle(seekerName)} ${partnerName}의 확고한 존재감과 흔들림 없는 태도에서 강한 이끌림을 경험합니다.`;
      emotionalMeaning = `어떤 상황에서도 나만을 최우선으로 지지해 주는 세상에서 가장 확실한 '내 편'이라는 확신을 갈망하기 때문입니다.`;
      scene = `주변의 복잡한 시선이나 세상의 평가 속에서도, 흔들림 없이 내 선택을 믿어주고 전폭적으로 힘을 실어주는 순간`;
      pairSpecificEffect = `외부의 어떤 어려움 앞에서도 둘만의 깊고 단단한 결속력을 발휘합니다.`;
      tensionBridge = `열정이 깊어질수록 상대의 사소한 시선 분산이나 반응 지연에 서운함이 커져 독점적 불안으로 이어질 수 있습니다.`;

      matchStrength = "strong";
      supportingReasons.push({
        text: `${partnerName}의 일관된 신뢰와 확고한 태도가 ${seekerName}에게 세상 누구보다 든든한 '내 편'이라는 깊은 안도감을 선사합니다.`,
    ...
    case "jeongjae": {
      recognition = `${topicParticle(seekerName)} ${partnerName}의 성실한 행동과 약속을 지키는 한결같은 태도에 깊은 신뢰를 느낍니다.`;
      emotionalMeaning = `말뿐인 다정함보다 일상의 현실을 정직하게 채워주는 예측 가능한 안정감에서 삶의 평온을 얻기 때문입니다.`;
      scene = `함께 중요한 일정을 조율할 때, 서로의 시간을 존중하며 사소한 디테일까지 성실하게 챙겨주는 모습을 마주할 때`;
      pairSpecificEffect = `일상의 모든 순간이 안정되고 투명하게 관리되는 단단한 생활 기반을 만듭니다.`;
      tensionBridge = `예상치 못한 일정 변동이나 상황의 변화 앞에서 규칙을 지나치게 고수하면, 상대에게 답답한 통제감으로 느껴질 수 있습니다.`;

      if (partnerStem === "gyeong" || partnerStem === "sin" || partnerDominantElement === "metal" || partnerDominantElement === "earth") {
        matchStrength = "strong";
        supportingReasons.push({
          text: `${subjectParticle(partnerName)} 보여주는 분명한 원칙과 투명한 실행력이 ${subjectParticle(seekerName)} 가장 필요로 하는 구체적인 일상의 안정감을 채워줍니다.`,
        ...
        supportingReasons.push({
          text: `${partnerName}의 성실한 노력이 ${seekerName}에게 현실적인 신뢰의 토대가 되어줍니다.`,
    ...
    case "pyeonjae": {
      recognition = `${topicParticle(seekerName)} ${partnerName}의 넓은 시야와 유쾌한 활력, 주저 없이 세상을 대하는 에너지에 매료됩니다.`;
      emotionalMeaning = `단조로운 일상에 갇히지 않고 함께 다채로운 세상을 경험하며 넓은 공간을 공유하는 자유로움을 바라기 때문입니다.`;
      scene = `주말에 계획에 없던 새로운 장소나 경험을 함께 제안받고, 설레는 마음으로 일상의 경계를 넓혀가는 순간`;
      pairSpecificEffect = `관계가 늘 생동감 넘치고 지루함 없이 함께 새로운 세상을 탐색해 나갑니다.`;
      tensionBridge = `새로운 경험을 추구하는 속도가 다를 때, 한 사람의 경쾌함이 다른 사람에게는 산만함이나 피로감으로 느껴질 수 있습니다.`;

      matchStrength = "strong";
      supportingReasons.push({
        text: `${partnerName}의 추진력과 생동감이 ${seekerName}의 일상에 활력을 불어넣고 함께 즐거운 경험을 확장해 나갈 수 있게 돕습니다.`,
    ...
    case "jeonggwan": {
      recognition = `${topicParticle(seekerName)} ${partnerName}의 정직하고 반듯한 태도, 매사에 중심을 지키는 품격에 마음을 엽니다.`;
      emotionalMeaning = `서로에게 부끄럽지 않은 신의를 지키며, 어디서나 서로를 존중해 주는 품격 있는 관계의 도리를 소중히 여기기 때문입니다.`;
      scene = `다른 사람들과 함께하는 자리에서도 서로를 깎아내리지 않고 든든하게 예의와 품격을 지켜주는 모습을 볼 때`;
      pairSpecificEffect = `주변의 신뢰를 받는 견고하고 떳떳한 관계의 틀을 세워냅니다.`;
      tensionBridge = `도리와 원칙을 지나치게 엄격하게 앞세우면, 솔직한 감정의 굴곡을 털어놓지 못하고 관계가 경직될 수 있습니다.`;

      matchStrength = "strong";
      supportingReasons.push({
        text: `${partnerName}의 단정하고 책임감 있는 태도가 ${seekerName}에게 높은 품격과 확신을 갖춘 신뢰를 선사합니다.`,
    ...
    case "pyeongwan": {
      recognition = `${topicParticle(seekerName)} ${partnerName}의 결단력과 묵직한 무게감, 위기 앞에서도 물러서지 않는 태도에 깊은 안도감을 느낍니다.`;
      emotionalMeaning = `힘든 현실에 혼자 맞서지 않아도 된다는 확신, 나를 지켜주고 함께 무게를 짊어져 줄 든든한 방패를 찾기 때문입니다.`;
      scene = `예상치 못한 곤란이나 위기가 닥쳤을 때, 피하지 않고 앞장서서 상황을 정돈하며 든든하게 어깨를 내어주는 순간`;
      pairSpecificEffect = `어떤 시련이 와도 둘이 함께라면 이겨낼 수 있다는 강력한 보호막을 형성합니다.`;
      tensionBridge = `위기를 해결하려는 강한 책임감이 때로는 상대에게 무언의 압박이나 통제로 전해질 수 있습니다.`;

      matchStrength = "strong";
      supportingReasons.push({
        text: `${partnerName}의 든든한 결단력과 무게감이 ${subjectParticle(seekerName)} 어떤 어려움 속에서도 안심하고 의지할 수 있는 방패가 되어줍니다.`,
    ...
    case "jeongin": {
      recognition = `${topicParticle(seekerName)} ${partnerName}의 다정한 시선과 모든 것을 편안하게 품어주는 온기에 마음이 무장해제됩니다.`;
      emotionalMeaning = `나의 연약함과 부족한 모습까지도 있는 그대로 수용받는 따뜻한 품에서 가장 깊은 정서적 안식을 찾기 때문입니다.`;
      scene = `밖에서 지치고 혼란스러운 하루를 보내고 돌아왔을 때, 잘잘못을 묻지 않고 조용히 곁을 지켜주며 따뜻한 온기를 건네받는 순간`;
      pairSpecificEffect = `마음의 모든 상처를 아물게 하는 깊은 정서적 치유와 쉼터를 공유합니다.`;
      tensionBridge = `상대의 무조건적인 수용에 지나치게 기대어 서운함을 속으로만 삭이다가 뒤늦게 토로할 위험이 있습니다.`;

      matchStrength = "strong";
      supportingReasons.push({
        text: `${partnerName}의 따뜻한 시선과 포용력이 ${seekerName}의 지친 마음을 온전히 누그러뜨려 주는 편안한 안식처가 됩니다.`,
    ...
    case "pyeonin": {
      recognition = `${topicParticle(seekerName)} ${partnerName}의 깊은 통찰력과 침착함, 말없이도 상황을 헤아려 주는 섬세함에 자연스럽게 눈길이 머뭅니다.`;
      emotionalMeaning = `자신의 복잡한 내면과 고독을 섣불리 재단하지 않고 묵묵히 품어주는 통찰력 있고 너그러운 여백을 바라기 때문입니다.`;
      scene = `수많은 생각과 감정이 뒤엉켜 말문이 막혔을 때, 다그치지 않고 차분한 눈빛으로 생각을 정리할 여백을 건네받는 순간`;
      pairSpecificEffect = `말하지 않아도 서로의 깊은 고독과 생각을 알아채는 특별한 정서적 교감을 이룹니다.`;
      tensionBridge = `생각이 많아질 때 각자의 내면으로 숨어버리면 대화가 단절되어 서로의 의중을 오해하기 쉽습니다.`;

      matchStrength = "strong";
      supportingReasons.push({
        text: `${partnerName}의 사려 깊은 통찰과 경청이 ${seekerName}의 복잡한 내면을 말없이도 헤아려 주는 특별한 정서적 교감을 만듭니다.`,
    ...
    case "siksin": {
      recognition = `${topicParticle(seekerName)} ${withParticle(partnerName)} 함께 있을 때 느껴지는 자연스럽고 부담 없는 편안함에 안도감을 느낍니다.`;
      emotionalMeaning = `어떤 평가나 조급한 요구 없이 나만의 속도를 너그럽게 지켜봐 주는 다정한 여유를 갈망하기 때문입니다.`;
      scene = `일과를 마치고 복잡한 생각에 지쳤을 때, 거창한 해결책 대신 편안한 웃음과 따뜻한 여유를 함께 나눌 때`;
      pairSpecificEffect = `일상의 긴장을 자연스럽게 녹여내는 포근한 일상성을 선물합니다.`;
      tensionBridge = `서로 편안함에 안주하여 현실적인 결정이나 깊은 대화를 미루다 보면 작은 불만이 누적될 수 있습니다.`;

      matchStrength = "strong";
      supportingReasons.push({
        text: `${withParticle(partnerName)} 함께 있을 때 느껴지는 자연스럽고 부담 없는 편안함이 ${seekerName}의 일상에 잔잔한 행복을 채워줍니다.`,
    ...
    case "sanggwan": {
      recognition = `${topicParticle(seekerName)} ${partnerName}의 틀에 박히지 않은 센스와 기민한 위트, 번뜩이는 관점에 매료됩니다.`;
      emotionalMeaning = `나의 솔직한 생각과 독특한 시선을 가식 없이 주고받을 수 있는 지적·감성적 교감을 갈망하기 때문입니다.`;
      scene = `예상치 못한 돌발 상황 앞에서도 유쾌한 대화와 번뜩이는 감각으로 긴장을 풀어내며 함께 웃음을 터뜨릴 때`;
      pairSpecificEffect = `지루할 틈 없는 신선한 자극과 역동적인 대화의 즐거움을 공유합니다.`;
      tensionBridge = `대화의 핑퐁이 과열되면 솔직한 피드백이 상대의 아픈 곳을 찌르는 날카로운 언어로 변할 수 있습니다.`;

      matchStrength = "strong";
      supportingReasons.push({
        text: `${partnerName}의 기민한 반응과 센스 있는 피드백이 ${seekerName}에게 지루할 틈 없는 신선한 자극과 매력을 선사합니다.`,
  ...
  if (partnerStemEl === "earth" && seekerStemEl === "metal") {
    supportingReasons.push({
      text: `${partnerName}의 넓고 차분한 품이 ${seekerName}의 명확한 판단과 결단력을 따뜻하게 받쳐주는 든든한 안정감을 더합니다.`,
    ...
    supportingReasons.push({
      text: `${partnerName}의 명료하고 깔끔한 기준이 ${seekerName}의 깊은 생각에 확실한 방향성을 잡아주는 역할을 합니다.`,
    ...
    supportingReasons.push({
      text: `${partnerName}의 깊은 통찰과 공감이 ${seekerName}의 새로운 시도와 도전에 부드러운 힘을 실어줍니다.`,
    ...
    supportingReasons.push({
      text: `${partnerName}의 생동감 넘치는 호기심이 ${seekerName}의 따뜻한 열정을 한층 밝게 피워 올립니다.`,
  ...
  if (
    ...
    supportingReasons.push({
      text: `두 사람의 내면 리듬이 맞물리면서, 함께할 때 마음의 긴장이 풀리고 자연스러운 정서적 일체감을 느낍니다.`,
  ...
  const narrativeUnit: AttractionNarrativeUnit = {
    ...
    recognition: sanitizeKoreanParticles(recognition, [seekerName, partnerName]),
    emotionalMeaning: sanitizeKoreanParticles(emotionalMeaning, [seekerName, partnerName]),
    partnerEvidence: supportingReasons.map((r) => sanitizeKoreanParticles(r.text, [seekerName, partnerName])),
    scene: sanitizeKoreanParticles(scene, [seekerName, partnerName]),
    pairSpecificEffect: sanitizeKoreanParticles(pairSpecificEffect, [seekerName, partnerName]),
    tensionBridge: sanitizeKoreanParticles(primaryCaution, [seekerName, partnerName]),
    evidenceIds: [
  ...
  const synthesisNarrative = sanitizeKoreanParticles(
  ...
  return {
    ...
    preferenceSource,
    preferenceMeaning: {
      text: sanitizeKoreanParticles(seekerSpousePalace.partnerExpectation, [seekerName, partnerName]),
    ...
    supportingReasons: supportingReasons.map((r) => ({
      ...r,
      text: sanitizeKoreanParticles(r.text, [seekerName, partnerName]),
    })),
