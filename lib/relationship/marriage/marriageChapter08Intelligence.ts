...
export function buildMarriageChapter08Intelligence(
  ...
  const headlineA = currentRespA.responseProfile.pressureResponse
    ? "서두르기보다 차분히 틀과 기준을 세우는 시기"
    : currentRespA.responseProfile.changeResponse
    ? "조건을 신중하게 확인하며 안정적으로 응대하는 시기"
    : "내실을 다지며 신중하게 흐름을 관망하는 시기";

  const descA = `${nameA}님은 주변 상황에서 조급히 결정하기보다, 계획과 조건을 먼저 확인하며 기준을 잡으려는 때입니다.`;

  const headlineB = currentRespB.responseProfile.actionResponse
    ? "새로운 시도와 실행을 주도적으로 탐색하는 시기"
    : currentRespB.responseProfile.changeResponse
    ? "새로운 흐름을 유연하게 받아들이는 시기"
    : "다음 흐름을 모색하며 가볍게 시도해보는 시기";

  const descB = `${nameB}님은 새로운 가능성을 탐색하며 가볍고 신속하게 움직여보려는 의욕이 커지는 때입니다.`;

  const pairHeadline = `한 사람은 차분히 기준을 잡고, 다른 사람은 새로운 시도를 모색하는 시기`;
  const pairDesc = `${nameA}님과 ${nameB}님이 생각을 결정하고 움직이는 템포에 차이가 있으므로, 서로의 속도를 인정해주는 호흡이 중요합니다.`;
  ...
  if (currentPairState.pairState === "DIFFERENT_SPEED" || currentPairState.pairState === "SUPPORTIVE_ASYMMETRY") {
    section02RelationshipThemes.push({
      ...
      title: "결정을 언제 함께 확정할지가 중요해지는 해",
      description: `${nameA}님과 ${nameB}님의 결정 템포가 다를 수 있으므로, 아이디어를 꺼내는 시점과 실제 결정을 확정하는 시점을 구분하여 논의하면 훨씬 편안해집니다.`,
  ...
  if (currentPairState.pairState === "ONE_PARTNER_TRANSITION" || currentPairState.pairState === "MUTUAL_TRANSITION") {
    ...
    section02RelationshipThemes.push({
      id: "theme_transition",
      title: "앞으로 나아갈 삶의 우선순위를 재정돈하는 해",
      description: `${changingPartnerName}님이 익숙했던 흐름에서 벗어나 다음 단계를 구상함에 따라, 두 사람이 앞으로 중요하게 둘 가치와 역할을 다듬어갈 필요가 커집니다.`,
  ...
  if (section02RelationshipThemes.length < 2) {
    section02RelationshipThemes.push({
      ...
      title: "지금 잘 굴러가는 방식을 지키면서도 새로운 선택지를 여는 균형",
      description: "기존에 구축해 둔 안정적인 기준을 지키면서도, 상대가 건네는 새로운 제안이나 시도를 너무 빨리 닫지 않고 들어보는 유연함이 도움이 됩니다.",
  ...
  const section03ThreeYearForecast: YearlyForecastCard[] = targetYears.map((yr) => {
    ...
    if (isShiftingYear && yr <= 2027) {
      yearLabel = "익숙한 방식을 정돈하고 다음 방향을 가늠하는 해";
      badge = "다음 방향 정돈";
    } else if (yr >= 2028 && (rB.timingContext.backgroundThemes.includes("wealth_theme_background") || state.pairState === "ALIGNED_MOMENTUM")) {
      yearLabel = "준비하던 방향이 실제 행동과 확장으로 이어지는 해";
      badge = "새 방향 실행";
    } else if (state.pairState === "SHARED_STABILITY") {
      yearLabel = "안정적으로 맞춰나가는 해";
      badge = "안정적 순항";
    } else if (state.pairState === "DUAL_PRESSURE") {
      yearLabel = "서로의 짐을 나누어 지는 해";
      badge = "짐 나누기";
    }

    // Individual Summaries Differentiated Per Year (No internal engine jargon)
    let summaryA = "서두르기보다 기준을 점검하며 안정적으로 대처하는 해";
    if (yr === 2026) {
      summaryA = "주어지는 변화 속에서 서두르지 않고 먼저 틀과 기준을 잡는 해";
    } else if (yr === 2027 || seunFactA?.tenGodCode === "jeonggwan" || seunFactA?.tenGodCode === "pyeongwan") {
      summaryA = "책임과 역할을 조율하며 신중하게 다음 순서를 정돈하는 해";
    } else if (yr === 2028 || seunFactA?.tenGodCode === "pyeonin" || seunFactA?.tenGodCode === "jeongin") {
      summaryA = "외부 요구가 완화되며 내면의 안식을 찾고 한숨 돌리는 해";
    }

    let summaryB = "가능성을 탐색하며 가볍게 시도해보는 해";
    if (yr === 2027 && isB_Shifting) {
      summaryB = "익숙했던 방식을 정리하고 앞으로 나아갈 다음 방향을 준비하는 해";
    } else if (yr === 2028 && rB.timingContext.backgroundThemes.includes("wealth_theme_background")) {
      summaryB = "준비해온 새로운 방향이 실제 행동으로 이어지며 활동 범위를 본격적으로 넓히는 해";
    } else if (rB.responseProfile.actionResponse) {
      summaryB = "새로운 시도와 아이디어를 가볍게 실행해보는 해";
    }

    // Couple Rhythm Summary (Absorbs speed contrast & pair dynamic)
    let summaryPair = `${nameA}님은 차분한 기준을 지키고 ${nameB}님은 탐색 의욕이 커지므로, 결정 시점을 넉넉하게 잡는 것이 좋습니다.`;
    if (isShiftingYear && yr === 2027) {
      const partnerName = isB_Shifting ? nameB : nameA;
      summaryPair = `${partnerName}님이 다음 방향을 구상하는 과정에서, 두 사람이 앞으로 중요하게 둘 우선순위를 함께 맞춰보는 호흡이 부각됩니다.`;
    } else if (yr === 2028) {
      summaryPair = "새롭게 선택한 방향이 실제 일상으로 자리 잡으며, 달라진 템포에 두 사람이 안정적으로 적응해나가는 시기입니다.";
    } else if (state.pairState === "SHARED_STABILITY") {
      summaryPair = "두 사람 모두 안정을 바탕으로 무난하고 편안하게 호흡을 맞춰가는 해입니다.";
    }
  ...
  if (turningPointYearState) {
    ...
    const headline = `${tpYr}년, ${changingName}님이 새로운 방향을 구상하며 찾아오는 주요 변곡점`;
    const reason = `${tpYr}년에는 삶의 전환점이 다가오면서, 두 사람이 앞으로 중요하게 둘 방향과 우선순위를 함께 다시 맞춰볼 필요가 커지는 시기입니다.`;

    section04TurningPoint = {
      year: tpYr,
      headline,
      reason,
      forPersonA: `${nameA}님은 변화를 겪는 상대에게 조급한 확답을 요구하기보다, 차분하게 조건과 순서를 살펴주는 버팀목이 됩니다.`,
      forPersonB: `${nameB}님은 새로운 흐름으로 넘어가는 과정에서 조급함을 내려놓고 상대와의 협의 템포를 맞춥니다.`,
      forPair: "이 시기는 한 사람의 판단만으로 추진하기보다, 부부가 앞으로 함께 나아갈 기준을 함께 논의하는 결정적 계기가 됩니다.",
    };
  ...
  const adviceA = `${nameA}님은 부담이나 고민이 생길 때 혼자 모든 조건을 완벽히 정리하려 하기보다, 생각하는 초기 단계부터 상대와 가볍게 대화를 시작해 보세요.`;
  const adviceB = `${nameB}님은 새로운 아이디어가 떠올랐을 때 곧바로 실행안부터 내놓기보다, 왜 해보고 싶은지 그 이유와 계기부터 다정하게 이야기해 주세요.`;
  const advicePair = "한 사람이 먼저 가능성을 열고 다른 사람이 현실적인 조건을 점검하는 순서로 움직이면, 서로의 속도 차이를 자연스러운 역할 분담으로 쓰기 쉬워집니다.";
