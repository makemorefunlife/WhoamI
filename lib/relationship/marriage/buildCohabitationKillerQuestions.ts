...
function resolveIntroLine(signals: CohabitationKillerSignals): string {
  const hasPsych =
  ...
  if (hasPsych) {
    return "아래 질문은 만세력(사주)과 11축 설문을 함께 읽어 뽑았어요. '우리만 그런 거 아니야?'가 나오면 해당 섹션을 펼쳐 보세요.";
  }
  return "아래 질문은 만세력(사주) 규칙으로 뽑았어요. 설문을 완료하면 심리 축과 교차 검증된 질문이 더 정밀해집니다.";
}
...
export function buildCohabitationKillerQuestions(params: {
  ...
  const questions = dedupeByTopic(
    buildCohabitationKillerRuleCandidates(signals),
  )
    .sort((a, b) => b.priority - a.priority)
    .slice(0, MAX_KILLER_QUESTIONS);

  return {
    intro_line: resolveIntroLine(signals),
    questions,
  };
}
