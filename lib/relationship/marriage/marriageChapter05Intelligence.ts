...
export type MentalLoadSection = {
  title: string;
  noticer: string;
  rememberer: string;
  doer: string;
  closer: string;
  frictionPointNote?: string;
};
...
export type MarriageChapter05Intelligence = {
  ...
  mentalLoad: MentalLoadSection; // 07
...
export function buildMarriageChapter05Intelligence(params: {
  ...
  // ---------------------------------------------------------------------------
  const propA = (countsA["식상"] ?? 0) + ((axesA.growth ?? 50) > 55 ? 1 : 0);
  const propB = (countsB["식상"] ?? 0) + ((axesB.growth ?? 50) > 55 ? 1 : 0);

  const checkA_dec = (countsA["재성"] ?? 0) + ((axesA.practicality ?? 50) > 55 ? 1 : 0);
  const checkB_dec = (countsB["재성"] ?? 0) + ((axesB.practicality ?? 50) > 55 ? 1 : 0);

  const brakeA = (countsA["관성"] ?? 0) + ((axesA.stability ?? 50) > 55 ? 1 : 0);
  const brakeB = (countsB["관성"] ?? 0) + ((axesB.stability ?? 50) > 55 ? 1 : 0);

  const pushA = (countsA["비겁"] ?? 0) + ((axesA.decision_style ?? 50) > 55 ? 1 : 0);
  const pushB = (countsB["비겁"] ?? 0) + ((axesB.decision_style ?? 50) > 55 ? 1 : 0);
  ...
  const majorMoneyDecisions: MajorMoneyDecisionsSection = {
    ...
    optionProposer: propA >= propB ? `${nameA} (가능성 탐색)` : `${nameB} (가능성 탐색)`,
    numberChecker: checkA_dec >= checkB_dec ? `${nameA} (숫자 검토)` : `${nameB} (숫자 검토)`,
    riskBrake: brakeA >= brakeB ? `${nameA} (안전판 역할)` : `${nameB} (안전판 역할)`,
    commitPusher: pushA >= pushB ? `${nameA} (최종 결정)` : `${nameB} (최종 결정)`,
  ...
  // 07. MENTAL_LOAD
  // ---------------------------------------------------------------------------
  const mlNotice = (axesA.structure ?? 50) >= (axesB.structure ?? 50) ? `${nameA} (우선 포착)` : `${nameB} (우선 포착)`;
  const mlRemember = (axesA.practicality ?? 50) >= (axesB.practicality ?? 50) ? `${nameA} (기억 및 유지)` : `${nameB} (기억 및 유지)`;
  const mlDo = (axesA.energy_style ?? 50) >= (axesB.energy_style ?? 50) ? `${nameA} (직접 집행)` : `${nameB} (직접 집행)`;
  const mlClose = (axesA.self_control ?? 50) >= (axesB.self_control ?? 50) ? `${nameA} (마무리 점검)` : `${nameB} (마무리 점검)`;

  const mentalLoad: MentalLoadSection = {
    title: isEn ? "07. Mental Load Workflow" : "07. 보이지 않는 집안일은 누구 머릿속에 있을까?",
    noticer: mlNotice,
    rememberer: mlRemember,
    doer: mlDo,
    closer: mlClose,
    frictionPointNote: "일의 시작(알아차림)부터 마무리까지 한 사람에게 짐이 쏠리지 않도록, '알아차린 사람이 직접 처리까지 완결'하는 바통 터치가 도움이 됩니다.",
  };
  ...
  const householdMapEnding: HouseholdMapEnding = {
    ...
    bigMoneyDecisionSummary: `대형 지출 결정: ${majorMoneyDecisions.decisionPatternSummary}`,
    lifeCompetenceSummary: `일상 생활력 시너지: ${practicalLifeCompetence.pairSynergyInsight}`,
    mentalLoadSummary: `보이지 않는 책임 관리: ${mentalLoad.frictionPointNote ?? "상호 균형적 관리"}`,
  };
...
export function createDefaultMarriageChapter05Intelligence(params: {
  ...
  return {
    ...
    householdMapEnding: {
      ...
      bigMoneyDecisionSummary: "대형 지출 결정: 한 쪽의 가치제안과 다른 한 쪽의 숫자리뷰 상호검토",
      lifeCompetenceSummary: "일상 생활력 시너지: 각자 강점이 있는 집안일 영역에서 주도성 발휘",
      mentalLoadSummary: "보이지 않는 책임 관리: 알아차림과 실 집행 분담 간 명확한 공유 유지",
    },
  };
}
