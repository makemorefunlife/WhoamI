/**
 * Friend premium 클라이언트 응답용 — report.context_output 제거 사본.
 * DB/로그에 넣는 payload는 이 함수를 거치지 않는다(원본 유지).
 * 원본 객체는 mutate하지 않는다.
 */
export function stripFriendContextOutputForClient<T extends { report?: unknown }>(
  payload: T,
): T {
...
export function omitFriendContextOutputFromReport<T extends object>(
  report: T,
): T {
  if (!report || typeof report !== "object" || Array.isArray(report)) {
    return report;
