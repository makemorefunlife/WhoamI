...
const GENIUS_TITLES: Record<string, string> = {
  wood: "🌿 자유로운 탐험가형 천재",
  fire: "🔥 아이디어 폭발형 천재",
  earth: "🧸 든든한 안정감 천재",
  metal: "💎 명확한 기준형 천재",
...
export function buildFamilyKillerSections(params: {
  ...
  const { childNickname, parentNickname, parentRole, familyPairAnalysis, tenGod, masterScores } =
    params.ctx;
  const sig = familyPairAnalysis.scoringSignals;
  const childSig = familyPairAnalysis.childSignals;
  const roleKo = parentRole === "mother" ? "엄마" : "아빠";

  const geniusTitle =
    GENIUS_TITLES[childSig.dominantArchetype] ?? GENIUS_TITLES.earth!;

  const commMap: Record<string, string> = {
    expressive: `${childNickname}는 생각이 떠오르면 바로 말로 꺼내는 편이에요. 머릿속을 길게 굴리기보다 '지금 느끼는 것'을 먼저 공유합니다.`,
    analytical: `${childNickname}는 논리와 사실로 마음을 정리해요. 감정만 받아들이기보다 '왜 그랬는지' 이해해야 안심합니다.`,
    sensitive: `${childNickname}는 말투·표정·분위기에 예민하게 반응해요. 큰 소리보다 부드러운 톤이 훨씬 잘 들립니다.`,
    steady: `${childNickname}는 천천히, 그러나 깊게 말하는 타입이에요. 즉답을 재촉하면 입을 닫을 수 있습니다.`,
    playful: `${childNickname}는 유머와 장난으로 마음을 여는 경우가 많아요. 너무 진지한 분위기는 부담이 될 수 있습니다.`,
  };

  let hiddenSensitivity = `${childNickname}는 겉으론 괜찮아 보여도, 작은 비난에도 오래 마음에 담아두는 편일 수 있어요.`;
  ...
  let attentionFocus = `${childNickname}는 관심 분야에 몰입하면 시간 가는 줄 모르는 타입이에요.`;
  ...
  let hiddenGenius = `${childNickname}는 지금은 느리거나 엉뚱해 보일 때가 있어도, 자기만의 속도로 크게 자랄 잠재력이 있어요.`;
  ...
  let oneLine = `${childNickname} & ${parentNickname} — 서로 다른 리듬을 이해할수록 깊어지는 ${roleKo}-자녀 관계`;
  if (masterScores.bond >= 70 && masterScores.risk < 40) {
    oneLine = `${childNickname} & ${parentNickname} — 죽고 못 사는 찰떡궁합에 가까운 따뜻한 유대`;
  } else if (sig.hasStrongParentChildClash || sig.hasDayMonthTensionBond) {
    oneLine = `${childNickname} & ${parentNickname} — 사랑하지만 한 걸음 떨어져 지켜봐야 할 소중한 고슴도치 관계`;
  } else if (masterScores.synergy >= 65) {
    oneLine = `${childNickname} & ${parentNickname} — 서로의 빈틈을 채워 주는 성장 파트너`;
  }

  let harmonyLine = "서로 다른 매력을 가진 두 사람 — 이해하면 시너지가 큽니다.";
  ...
  let favoritism = `${parentNickname}는 ${childNickname}에게 특별히 서툰 편일 수 있어요. 의도와 다르게 냉정해 보일 때, 먼저 감정을 확인해 주세요.`;
  ...
  if (yearClashDay) focusAreas.push("자존감·정체성");
  if (yearClashMonth) focusAreas.push("학교·친구 관계");
  if (!focusAreas.length) focusAreas.push("새로운 도전", "감정 표현");

  let growthChallenge = `${childNickname}는 올해 '내가 누구인지'를 스스로 정의하려는 시기를 지나고 있어요.`;
  ...
  let filialReward = `${childNickname}는 커서 ${parentNickname}에게 정서적 위로를 건네는 타입으로 자랄 가능성이 있어요.`;
