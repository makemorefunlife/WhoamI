...
const ROLE_DESCRIPTION: Record<Locale, Record<FamilyPsychRole, (nickname: string) => string>> = {
  ...
  "ko-KR": {
    fixer: (n) =>
      `${josaEunNeun(n)} 집에서 문제가 생기면 먼저 나서서 해결하려는 편이에요. 큰 강점이지만, 항상 혼자 짊어질 필요는 없다는 걸 알려주세요 — 가끔은 부모가 해결하는 모습도 보여주세요.`,
    mediator: (n) =>
      `${josaEunNeun(n)} 가족 사이에서 분위기를 살피고 갈등을 조율하는 역할을 자연스럽게 맡아요. 그 노력을 직접 고맙다고 말해주세요 — 안 그러면 티 안 나게 지나가기 쉬워요.`,
    martyr: (n) =>
      `${josaEunNeun(n)} 자기 마음은 뒤로 하고 가족을 먼저 챙기려 애쓰는 편이에요. 무언가 부탁하기 전에 "너는 요즘 어때?"부터 먼저 물어봐 주세요 — 겉보기보다 인정받고 싶은 마음이 커요.`,
    independent: (n) =>
      `${josaEunNeun(n)} 가족에게 기대기보다 혼자 해결하려는 편이에요. 좋은 힘이지만, 도움을 청해도 괜찮다는 걸 느끼게 해주세요 — 독립이 거리감이 되지 않도록요.`,
    emotional_dump: (n) =>
      `${josaEunNeun(n)} 다른 가족의 감정을 조용히 받아주다 정작 자기 마음은 못 챙기기 쉬워요. 집안일이 있을 때만이 아니라, 그냥 "요즘 마음은 어때?"라고 물어봐 주세요.`,
    puppy: (n) =>
      `${josaEunNeun(n)} 집안 분위기를 밝게 띄우며 인정받고 싶어하는 편이에요. 분위기를 띄울 때만이 아니라 존재 자체를 칭찬해 주세요 — 밝음이 노력이 되지 않도록요.`,
  },
};
