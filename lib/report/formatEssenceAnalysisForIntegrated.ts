...
function formatShinsalSection(
  ...
  if (!shinsals?.length) {
    return "(해당 신살 없음 — Essence 분석의 다른 신호만 사용)";
  }
  return shinsals
    .map((s) => {
      const lines = [
...
/**
 * 통합 리포트 LLM 입력용 — Essence 분석(내면의 본질) 텍스트
...
주의: ${data.dayStemData.weakness_ko ?? ""}`
  ...
  return [
    block("원국(팔자)", pillars),
    block("일간 본질", dayStem),
    block("일지 본질", dayBranch),
    block("십성 분포", tenGods),
    block("12운성", stages),
    block("지장간", hiddenStems),
    block("합·충·형·파·해", relations),
    block("신살(특수 Essence 신호) — 반드시 해석에 반영", shinsals),
    "",
    "[작성 지침 — LLM용]",
    "- 위 내용은 'Essence 분석' 입력이다. 본문에 사주·팔자·십성·신살·천을귀인 등 전문 용어를 쓰지 말 것.",
    "- 신살 섹션의 의미·강점·주의를 일상 행동·관계·결정 패턴으로 풀어 Part 1~5에 녹일 것.",
    "- 근거를 말할 때 '사주에 따르면' 대신 'Essence 분석에 따르면' 또는 아예 출처 없이 자연스럽게 서술할 것.",
  ].join("\n\n");
}
...
export function sajuDataToIntegratedSummary(
  ...
  if (sajuData == null) return "(Essence 분석 데이터 없음)";
  if (typeof sajuData === "string") return sajuData;
  return formatEssenceAnalysisForIntegrated(sajuData as SajuDataForIntegrated);
}
