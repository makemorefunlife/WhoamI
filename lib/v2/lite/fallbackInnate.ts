...
export function buildInnateSelfLiteFallback(
  input: InnateSelfLiteInputPayload,
): InnateSelfLiteReport {
  const dm = input.day_master_signals;
  const db = input.day_branch_signals;
  const top = [...PRIMARY_AXIS_KEYS]
    .map((k) => ({
      key: k,
      score: input.innate_self_profile.primary_axes[k] ?? 50,
    }))
  ...
  const personality =
    dm.metaphor_ko ||
    dm.strength_ko ||
    "새로운 자극과 자율을 중시하는 기질이 기본에 깔려 있어요.";
  const relation =
    db.meaning_ko ||
    db.strength_ko ||
