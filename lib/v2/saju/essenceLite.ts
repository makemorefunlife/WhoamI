...
const ESSENCE_LITE_SPREAD_GAIN = 2.4;
...
export type EssenceLiteInput = {
  birthDate: string;
  birthTime?: string | null;
  birthTimeUnknown?: boolean;
};

export type EssenceSelfLiteProfile = {
...
/**
 * Lite Essence — 십성 raw 합산 후 spread gain (docs/v2/saju/04 기반 MVP)
 * TODO: 오행·계절·합충 가중 추가 시 ESSENCE_LITE_SPREAD_GAIN 재튜닝
 */
export function calculateEssenceSelfLite(
  input: EssenceLiteInput,
): EssenceSelfLiteProfile {
  ...
  const primary_axes = Object.fromEntries(
    PRIMARY_AXIS_KEYS.map((key) => [
      key,
      Math.round(
        clamp(50 + raw[key] * ESSENCE_LITE_SPREAD_GAIN, 0, 100),
      ),
    ]),
